...
Define how built-in, user, and configured skills are synchronized into the Agent runtime skills directory so that packaged `resources/skills` and `%AppData%/omni-worker/claude/skills` remain consistent without mutating on every Agent query.
